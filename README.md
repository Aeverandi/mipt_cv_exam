...
```
Первый раз загружается долго, потому что локально загружается более 4Гб модель *Stable Diffusion* (модель *Yolo8* тоже загружается, но значительно быстрее). Интерфейс реализован на *[Streamlit](https://streamlit.io)*.
